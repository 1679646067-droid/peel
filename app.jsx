...
const PORTFOLIO_DATA = {
  ...
  shorts: [
    { 
      ...
      shares: "3.2w", 
      title: "AI换脸特效展示",
      // 注: 因为跨域限制，抖音/小红书无法直接内嵌iframe，推荐使用自己下载好的竖屏MP4直链
    ...
    <>
      ...
      <div
        className={`fixed pointer-events-none z-[9999] w-12 h-12 rounded-full border flex items-center justify-center mix-blend-screen transition-all duration-300 ease-out hidden md:flex ${
          isClicking 
            ? 'scale-50 border-pink-500 bg-pink-500/20' 
            : 'scale-100 border-cyan-500/50'
        }`}
      ...
      <div
        className={`fixed pointer-events-none z-[9999] w-2 h-2 rounded-full mix-blend-screen transition-all duration-150 ease-out hidden md:block ${
          isClicking 
            ? 'bg-cyan-400 shadow-[0_0_20px_#22d3ee] scale-150' 
            : 'bg-pink-500 shadow-[0_0_15px_#ec4899] scale-100'
        }`}
  ...
  return (
    <div className="min-h-screen relative text-white pt-32 pb-20 px-4 flex flex-col items-center">
      ...
      <div className="relative z-10 max-w-6xl w-full grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 group relative aspect-video rounded-3xl overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(34,211,238,0.1)] bg-black">
          <video 
            ref={videoRef}
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[90%] py-4 px-8 rounded-2xl bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20">
            <div className="flex items-center gap-6 w-2/3">
              ...
              <div 
                className="h-2 w-full max-w-md bg-white/20 rounded-full relative overflow-hidden hidden md:block cursor-pointer group/progress"
                onClick={handleSeek}
              >
                <div 
                  className="absolute left-0 top-0 h-full bg-cyan-500 shadow-[0_0_10px_#06b6d4] transition-all duration-75" 
        ...
        <div className="flex flex-col gap-4">
          ...
          {PORTFOLIO_DATA.videos.map(v => (
            <button 
              ...
              className={`p-3 rounded-xl border transition-all duration-300 flex gap-4 items-center ${currentVideo.id === v.id ? 'bg-cyan-500/10 border-cyan-500/50' : 'bg-white/5 border-white/5 hover:border-white/20'}`}
            >
              <div className="w-16 h-10 bg-zinc-800 rounded-md overflow-hidden shrink-0">
                <img src={v.thumbnail} className="w-full h-full object-cover" onError={(e) => e.target.style.display='none'}/>
              </div>
...
const MusicPage = () => {
  ...
  const handleNext = () => {
    ...
    const nextIndex = (currentIndex + 1) % PORTFOLIO_DATA.music.length;
    setCurrentTrack(PORTFOLIO_DATA.music[nextIndex]);
    setIsPlaying(true);
  };

  const handlePrev = () => {
    const currentIndex = PORTFOLIO_DATA.music.findIndex(m => m.id === currentTrack.id);
    const prevIndex = (currentIndex - 1 + PORTFOLIO_DATA.music.length) % PORTFOLIO_DATA.music.length;
    setCurrentTrack(PORTFOLIO_DATA.music[prevIndex]);
    setIsPlaying(true);
  };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center transition-all duration-1000 bg-black">
      ...
      <div 
        className="absolute inset-0 transition-all duration-1000 scale-110 blur-3xl opacity-30"
      ...
      <div className="relative z-10 w-full max-w-5xl px-8 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
        ...
        <div className="bg-white/5 backdrop-blur-3xl p-8 rounded-3xl border border-white/10">
          <div className="flex justify-center gap-10 items-center mb-12 text-white">
            <button onClick={handlePrev} className="text-gray-400 hover:text-white transition-colors active:scale-90"><SkipBack size={28} /></button>
            <button 
              onClick={togglePlay}
              className="w-20 h-20 rounded-full bg-white text-black flex items-center justify-center hover:bg-pink-400 hover:text-white transition-all transform active:scale-95 shadow-xl"
            >
              {isPlaying ? <Pause size={32} /> : <Play size={32} fill="currentColor" />}
            </button>
            <button onClick={handleNext} className="text-gray-400 hover:text-white transition-colors active:scale-90"><SkipForward size={28} /></button>
          </div>
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
            {PORTFOLIO_DATA.music.map(m => (
              <div 
                ...
                className={`flex items-center justify-between p-4 rounded-xl cursor-pointer transition-all ${currentTrack.id === m.id ? 'bg-pink-500/20 text-pink-400 border border-pink-500/30' : 'text-gray-400 hover:bg-white/5 border border-transparent'}`}
              >
                <div className="flex items-center gap-4">
                  <span className="text-[10px] font-mono opacity-50 italic">#{m.id}</span>
                  <span className="text-sm font-light">{m.title}</span>
                </div>
                {currentTrack.id === m.id && isPlaying && <div className="flex gap-0.5 items-end h-3">
                  <div className="w-0.5 bg-pink-400 animate-[bounce_1s_infinite_0s]" style={{height:'60%'}} />
                  <div className="w-0.5 bg-pink-400 animate-[bounce_1s_infinite_0.2s]" style={{height:'100%'}} />
                  <div className="w-0.5 bg-pink-400 animate-[bounce_1s_infinite_0.4s]" style={{height:'40%'}} />
                </div>}
              </div>
            ))}
          </div>
          ...
          <div key={short.id} className="w-full max-w-[300px] group mx-auto flex flex-col">
            <div className="flex justify-between items-center mb-4 px-2">
              ...
              <div className="flex gap-1">
                ...
                <div className="w-1.5 h-1.5 rounded-full bg-orange-500 opacity-30" />
              </div>
            </div>

            {/* 视频窗口可点击播放 */}
            <div 
              onClick={() => handleTogglePlay(short.id)}
              className="aspect-[9/16] bg-zinc-900 rounded-[2rem] border-[4px] border-[#1a1a1a] overflow-hidden relative shadow-2xl transition-transform duration-500 group-hover:-translate-y-2 group-hover:shadow-orange-500/20 cursor-pointer"
            >
              {short.videoUrl && (
                <video 
                  src={short.videoUrl} 
                  className="w-full h-full object-cover"
                  autoPlay={playingId === short.id}
                  loop
                  muted={false}
                  playsInline
                />
              ...
              {playingId !== short.id && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-[2px] transition-opacity">
                  <div className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/20 mb-4 group-hover:scale-110 transition-transform">
              ...
              <div className="absolute right-3 bottom-20 flex flex-col gap-6 items-center bg-black/40 backdrop-blur-md p-3 rounded-full border border-white/10 z-10">
                <div className="flex flex-col items-center group/icon cursor-pointer">
                  <Heart size={20} className="mb-1 group-hover/icon:text-red-500 transition-colors" />
                  <span className="text-[10px] font-mono">{short.likes}</span>
                </div>
                <div className="flex flex-col items-center group/icon cursor-pointer">
                  <MessageCircle size={20} className="mb-1 group-hover/icon:text-orange-400 transition-colors" />
                  <span className="text-[10px] font-mono">{short.plays}</span>
                </div>
                <div className="flex flex-col items-center group/icon cursor-pointer">
                  <Share2 size={20} className="mb-1 group-hover/icon:text-blue-400 transition-colors" />
                  <span className="text-[10px] font-mono">{short.shares}</span>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-[#050505] text-white flex items-center justify-center p-6">
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-20">
          ...
          <div className="space-y-8 mt-12">
            <div className="group flex items-center gap-6 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-sm font-light tracking-wide">{PORTFOLIO_DATA.contact.email}</p>
              </div>
            </div>
            <div className="group flex items-center gap-6 cursor-pointer">
              <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-all">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-1">WeChat</p>
                <p className="text-sm font-light tracking-wide">{PORTFOLIO_DATA.contact.wechat}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <input type="email" placeholder="EMAIL ADDRESS" className="bg-transparent border-b border-white/10 py-4 outline-none focus:border-cyan-500 transition-colors text-sm font-light tracking-widest uppercase" />
